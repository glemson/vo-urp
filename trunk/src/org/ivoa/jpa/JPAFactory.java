...
public final class JPAFactory extends SingletonSupport {
    ...
    protected void clear() {
        if (log.isWarnEnabled()) {
            log.warn("JPAFactory.clear : enter : " + this.pu);
        }
        if (getEmf() != null) {
            try {
                getEmf().close();
    ...
    protected void initialize() throws IllegalStateException {
        ...
        try {
            ...
            if (log.isWarnEnabled()) {
                log.warn("JPAFactory.initialize : connecting to " + targetDB + " ...");
            }
        ...
        if (log.isWarnEnabled()) {
            log.warn("JPAFactory.initialize : done.");
        }
        ...
        try {
            ...
            if (log.isWarnEnabled()) {
                log.warn("JPAFactory.initialize : test to create an entityManager instance ...");
            }

            em = getEm();

            if (log.isWarnEnabled()) {
                log.warn("JPAFactory.initialize : test : OK");
            }
        } finally {
            if (em != null) {
                em.close();
        ...
        if (log.isWarnEnabled()) {
            log.warn("JPAFactory.initialize : exit : OK");
        }
    }
