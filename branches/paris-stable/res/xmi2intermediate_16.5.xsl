...
This XSLT is intended to work on XMI generated with MagicDraw Community Edition v16.5.
...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:variable name="uml_namespace" select="'http://schema.omg.org/spec/UML/2.2'"/>
  ...
  <xsl:template match="*[@xmi:type='uml:Class']">
    ...
    <xsl:element name="objectType">
      ...
      <xsl:if test="//packagedElement/ownedAttribute[@xmi:type='uml:Property' and @association and @aggregation='composite' and @type = $xmiid]">
        <xsl:variable name="idref" select="//packagedElement/ownedAttribute[@xmi:type='uml:Property' and @association and @aggregation='composite' and @type = $xmiid]/../@xmi:id"/>
        <xsl:variable name="relation" select="//packagedElement/ownedAttribute[@xmi:type='uml:Property' and @association and @aggregation='composite' and @type = $xmiid]/@name"/>
      ...
      <xsl:for-each select="//packagedElement/ownedAttribute[@xmi:type='uml:Property' and @association and (not(@aggregation) or @aggregation='shared') and @type = $xmiid]">
  ...
  TODO fix ... produces 1 where v12.1 did 0..1
   -->
  
  <xsl:template name="multiplicity">
    <xsl:param name="lower"/>
    <xsl:param name="upper"/>
<xsl:message>lower = <xsl:value-of select="$lower"/> and upper = <xsl:value-of select="$upper"/></xsl:message> 
    <xsl:choose>
      <xsl:when test="not($lower) and not($upper)">
        <xsl:value-of select="'1'"/>
      </xsl:when>
      <xsl:when test="not($lower) and $upper/@value = '1'">
        <xsl:value-of select="'0..1'"/>
      </xsl:when>
      <xsl:when test="$upper/@value = '1' and $lower/@value = '1'">
        <xsl:value-of select="'1'"/>
      </xsl:when>
      <xsl:when test="$upper/@value = '*' and not($lower)">
