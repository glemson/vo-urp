...
object-relational mapping strategy, this script uses the "single-table" approach.
We create a single table for a complete inheritcance hierarchy.
Must contain columns representing all attribute/references on subclasses.

For documentation about the different mapping strategies supported by JPA, see for example:
http://en.wikibooks.org/wiki/Java_Persistence/Inheritance.
For this strategy in particular see
http://en.wikibooks.org/wiki/Java_Persistence/Inheritance#Single_Table_Inheritance
...
<!-- CREATE TABLES -->
    <xsl:variable name="file" select="concat($vendor,'/',$project_name,'_createTables_SingleTable.sql')"/>
...
<!-- CREATE VIEWS -->
    <xsl:variable name="file" select="concat($vendor,'/',$project_name,'_createViews_SingleTable.sql')"/>
    <xsl:message >Opening file <xsl:value-of select="$file"/></xsl:message>
    <xsl:result-document href="{$file}">
      <xsl:value-of select="$header"/>
...
&bl;&bl;<xsl:value-of select="$primaryKeyColumnName"/>&bl;  <xsl:value-of select="$IDGentype"/>
&cr; 
...
<xsl:template match="objectType"  mode="columns">
  <xsl:variable name="xmiid" select="@xmiid"/>
  <xsl:apply-templates select="." mode="container"/>
  <xsl:apply-templates select="attribute" />
  <xsl:apply-templates select="reference" />
  <xsl:for-each select="//objectType[extends/@xmiidref = $xmiid]">
    <xsl:apply-templates select="." mode="columns"/>
  </xsl:for-each>
